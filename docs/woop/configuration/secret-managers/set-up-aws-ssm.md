...
description: "Set up AWS SSM (Systems Manager) for WoopChain."
keywords:
  - docs
  - woopchain
  - woop
...
This article details the necessary steps to get the WoopChain up and running with
...
After successfully creating IAM Policy, the user needs to attach it to the EC2 instance that is running the WoopChain server.
...
* **Parameter Path** (arbitrary path that the secret will be placed in, for example `/woop/nodes`)

## Step 1 - Generate the secrets manager configuration

In order for the WoopChain to be able to seamlessly communicate with the AWS SSM, it needs to parse an already
generated config file, which contains all the necessary information for secret storage on AWS SSM.

To generate the configuration, run the following command:

```bash
woop secrets generate --type aws-ssm --dir <PATH> --name <NODE_NAME> --extra region=<REGION>,ssm-parameter-path=<SSM_PARAM_PATH>
```

Parameters present:
* `PATH` is the path to which the configuration file should be exported to. Default `./secretsManagerConfig.json`
* `NODE_NAME` is the name of the current node for which the AWS SSM configuration is being set up as. It can be an arbitrary value. Default `woop-node`
* `REGION` is the region in which the AWS SSM resides. This has to be the same region as the node utilizing AWS SSM.
* `SSM_PARAM_PATH` is the name of the path that the secret will be stored in. For example if `--name node1` and `ssm-parameter-path=/woop/nodes`
are specified, the secret will be stored as `/woop/nodes/node1/<secret_name>`

:::caution Node names
Be careful when specifying node names.

The WoopChain uses the specified node name to keep track of the secrets it generates and uses on the AWS SSM.
