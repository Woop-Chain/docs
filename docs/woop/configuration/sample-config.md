...
description: "Start the WoopChain server using a configuration file."
keywords:
  - docs
  - woopchain
  - woop
...
The command used to start the server with a config file: `woop server --config <config_file_name>`

## Export config file with default configuration
The configuration with default settings for the WoopChain server can be exported into a config file in either `yaml` or `json` file format.
...
Checkout [CLI Commands](/docs/woop/get-started/cli-commands) section to get information on how to use these parameters.
