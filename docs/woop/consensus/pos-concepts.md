...
implementation of the WoopChain.

The WoopChain Proof of Stake (PoS) implementation is meant to be an alternative to the existing PoA IBFT implementation,
...
the [Staking Smart Contract](https://github.com/Woop-Chain/staking-contracts/blob/main/contracts/Staking.sol).

This contract is pre-deployed whenever a PoS mechanism WoopChain chain is initialized, and is available on the address
`0x0000000000000000000000000000000000001001` from block `0`.

### Epochs

Epochs are a concept introduced with the addition of PoS to the WoopChain.
...
epoch blocks, see the [Epoch Blocks](/docs/woop/consensus/pos-concepts#epoch-blocks) section.
...
Currently, the minimum threshold for entering the validator set is staking `1 WOOC`
:::
...
**Epoch Blocks** are a concept introduced in the PoS implementation of IBFT in WoopChain.
...
The default size of an epoch is `100000` blocks in the WoopChain.

## Contract pre-deployment

The WoopChain _pre-deploys_
the [Staking Smart Contract](https://github.com/Woop-Chain/staking-contracts/blob/main/contracts/Staking.sol)
