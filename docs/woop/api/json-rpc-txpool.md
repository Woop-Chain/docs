...
id: json-rpc-txpool
title: TxPool
description: "List of TxPool JSON RPC commands for WoopChain."
keywords:
  - docs
  - woopchain
  - woop
...
````bash
curl  https://rpc.woop.ai/rpc-X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"txpool_content","params":[],"id":1}'
````

<JsonRpcTerminal method="txpool_content" params={[]} network="https://rpc.woop.ai/rpc"/>
...
````bash
curl  https://rpc.woop.ai/rpc-X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"txpool_inspect","params":[],"id":1}'
````

<JsonRpcTerminal method="txpool_inspect" params={[]} network="https://rpc.woop.ai/rpc"/>
...
````bash
curl  https://rpc.woop.ai/rpc-X POST -H "Content-Type: application/json" --data '{"jsonrpc":"2.0","method":"txpool_status","params":[],"id":1}'
````

<JsonRpcTerminal method="txpool_status" params={[]} network="https://rpc.woop.ai/rpc"/>
