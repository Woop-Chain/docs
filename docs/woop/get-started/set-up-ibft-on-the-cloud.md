...
The below guide will instruct you on how to set up a WoopChain network on a cloud provider for a production setup of your testnet or mainnet.

If you would like to setup a WoopChain network locally to quickly test the `woop` before doing a production-like setup, please refer to
**[Local Setup](/docs/woop/get-started/set-up-ibft-locally)**
:::
...
As the only part of the `woop` that needs to be exposed to other VMs is the libp2p server, simply allowing
all communication between VMs on the default libp2p port `1478` is enough.

## Overview

![Cloud setup](/.gitbook/assets/ibft-setup/cloud.svg)
...
Each of the 4 nodes will run on their own VM, as the idea of this guide is to give you a fully functional WoopChain network while keeping the validator keys private to ensure a trustless network setup.
...
To get up and running with WoopChain, you need to initialize the data folders, on each node:


````bash
node-1> woop secrets init --data-dir data-dir
````

````bash
node-2> woop secrets init --data-dir data-dir
````

````bash
node-3> woop secrets init --data-dir data-dir
````

````bash
node-4> woop secrets init --data-dir data-dir
````
...
`bootnode` is not a special instance of a WoopChain node. Every WoopChain node can serve as a `bootnode` and
every WoopChain node needs to have a set of bootnodes specified which will be contacted to provide information on how to connect with
...
And lastly, we need the `<node_id>` which we can get from the output of the previously ran command `woop secrets init --data-dir data-dir` command (which was used to generate keys and data directories for the `node 1`)
...
````bash
node-1> woop server --data-dir ./data-dir --chain genesis.json  --libp2p 0.0.0.0:1478 --nat <public_or_private_ip> --seal
````

To run the **second** client:

````bash
node-2> woop server --data-dir ./data-dir --chain genesis.json --libp2p 0.0.0.0:1478 --nat <public_or_private_ip> --seal
````

To run the **third** client:

````bash
node-3> woop server --data-dir ./data-dir --chain genesis.json --libp2p 0.0.0.0:1478 --nat <public_or_private_ip> --seal
````

To run the **fourth** client:

````bash
node-4> woop server --data-dir ./data-dir --chain genesis.json --libp2p 0.0.0.0:1478 --nat <public_or_private_ip> --seal
````
...
````
Currently, we only support `json` based configuration file, sample config file can be found **[here](/docs/woop/configuration/sample-config)**
...
````bash 
woop server --data-dir <directory_path> --chain <genesis_filename>  --libp2p <IPAddress:PortNo> --nat <public_or_private_ip>
````
For example, you can add **fifth** Non-validator client by executing the following command :

````bash
woop server --data-dir ./data-dir --chain genesis.json --libp2p 0.0.0.0:1478 --nat<public_or_private_ip>
````
:::
