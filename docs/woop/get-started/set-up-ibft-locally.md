...
The below guide will instruct you on how to set up a WoopChain network on your local machine for testing and development
purposes.

The procedure differs greatly from the way you would want to set up WoopChain network for a real use scenario on
a cloud provider: **[Cloud Setup](/docs/woop/get-started/set-up-ibft-on-the-cloud)**
...
Refer to [Installation](/docs/woop/get-started/installation) to install WoopChain.

## Overview

![Local Setup](/.gitbook/assets/ibft-setup/local.svg)

In this guide, our goal is to establish a working `woop` blockchain network working with [IBFT consensus protocol](https://github.com/ethereum/EIPs/issues/650).
...
````bash
woop secrets init --data-dir test-chain-1
````

````bash
woop secrets init --data-dir test-chain-2
````

````bash
woop secrets init --data-dir test-chain-3
````

````bash
woop secrets init --data-dir test-chain-4
````
...
```bash
woop secrets output --data-dir test-chain-4
```
...
`bootnode` is not a special instance of the woop node. Every woop node can serve as a `bootnode`, but
every woop node needs to have a set of bootnodes specified which will be contacted to provide information on how to connect with
...
And lastly, we need the `<node_id>` which we can get from the output of the previously ran command `woop secrets init --data-dir test-chain-1` command (which was used to generate keys and data directories for the `node1`)
...
WoopChain supports using DNS hostnames for the nodes configuration. This is a very helpful feature for cloud based deployments, as the node's ip may change due to various reasons.
...
````bash
woop genesis --consensus ibft --ibft-validators-prefix-path test-chain- --bootnode /ip4/127.0.0.1/tcp/10001/p2p/16Uiu2HAmJxxH1tScDX2rLGSU9exnuvZKNM9SoK3v315azp68DLPW --bootnode /ip4/127.0.0.1/tcp/20001/p2p/16Uiu2HAmS9Nq4QAaEiogE4ieJFUYsoH28magT7wSvJPpfUGBj3Hq 
````

What this command does:

* The `--ibft-validators-prefix-path` sets the prefix folder path to the one specified which IBFT in WoopChain can
...
For example, if we would like to premine 1000 WOOC to address `0x3956E90e632AEbBF34DEB49b71c28A83Bc029862` in our genesis block, then we would need to supply the following argument:
...
Because we are attempting to run a WoopChain network consisting of 4 nodes all on the same machine, we need to take care to 
...
````bash
woop server --data-dir ./test-chain-1 --chain genesis.json --grpc-address :10000 --libp2p :10001 --jsonrpc :10002 --seal
````

To run the **second** client:

````bash
woop server --data-dir ./test-chain-2 --chain genesis.json --grpc-address :20000 --libp2p :20001 --jsonrpc :20002 --seal
````

To run the **third** client:

````bash
woop server --data-dir ./test-chain-3 --chain genesis.json --grpc-address :30000 --libp2p :30001 --jsonrpc :30002 --seal
````

To run the **fourth** client:

````bash
woop server --data-dir ./test-chain-4 --chain genesis.json --grpc-address :40000 --libp2p :40001 --jsonrpc :40002 --seal
````
...
The structure of the genesis file is covered in the [CLI Commands](/docs/woop/get-started/cli-commands) section.

After running the previous commands, you have set up a 4 node WoopChain network, capable of sealing blocks and recovering
from node failure.
...
````bash 
woop server --config <config_file_path>
````
Example:

````bash
woop server --config ./test/config-node1.json
````
Currently, we support `yaml` and `json` based configuration files, sample config files can be found **[here](/docs/woop/configuration/sample-config)**
...
````bash 
woop server --data-dir <directory_path> --chain <genesis_filename> --grpc-address <portNo> --libp2p <portNo> --jsonrpc <portNo>
````
For example, you can add **fifth** Non-validator client by executing the following command :

````bash
woop server --data-dir ./test-chain --chain genesis.json --grpc-address :50000 --libp2p :50001 --jsonrpc :50002 
````
:::

:::info Specify the price limit
A WoopChain node can be started with a set **price limit** for incoming transactions.
...
````bash
woop server --price-limit 100000 ...
````
...
:::info WebSocket URL
By default, when you run the WoopChain, it generates a WebSocket URL based on the chain location.
...
````bash
wss://rpc-edgenet.woopchain.technology/ws
````
:::



## Step 5: Interact with the woop network
...
Follow this guide to issue operator commands to the newly built cluster: [How to query operator information](/docs/woop/working-with-node/query-operator-info) (the GRPC ports for the cluster we have built are `10000`/`20000`/`30000`/`40000` for each node respectively)
