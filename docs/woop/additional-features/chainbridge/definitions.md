...
![ChainBridge Architecture](/.gitbook/assets/chainbridge/architecture.svg)
...
![Workflow of ERC20 token transfer](/.gitbook/assets/chainbridge/erc20-workflow.svg)
...
Please make sure the accounts have enough native tokens to create transactions before starting. In WoopChain, you can assign premined balances to accounts when generating the genesis block.
...
## JSON-RPC URL for WoopChain PoS

For this guide, we’ll use https://rpctestnet.woop.ai/rpc, a public JSON-RPC URL provided by WoopChain, which may have traffic or rate-limits. This will be used only to connect with the WoopChain Mumbai testnet. We advise you to obtain your JSON-RPC URL by an external service like Infura because deploying contracts will send many queries/requests to the JSON-RPC.
