...
So far, we've set up bridge to exchange assets/data between WoopChain PoS and WoopChain chain. This section will guide you to set up an ERC20 bridge and send tokens between different blockchains.
...
We will send ERC20 Tokens from the WoopChain PoS chain to the WoopChain chain.
...
To transfer tokens to WoopChain chains, you will call `deposit`.
...
After the deposit transaction was successful, the relayer will get the event and vote for the proposal. It executes a transaction to send tokens to the recipient account in the WoopChain chain after the required number of votes are submitted. 
...
Once the execution transaction has been successful, you will get tokens in the WoopChain chain.

```bash
# Check the ERC20 balance in WoopChain chain
$ cb-sol-cli erc20 balance \
