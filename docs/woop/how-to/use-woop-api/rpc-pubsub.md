---
description: Using RPC Pub/Sub with WoopChain WebSockets
tags:
  - private networks
---
...
!!!important
    ...
    Unlike other [WoopChain API methods](../../reference/api/index.md), you cannot call
...
The new headers notification returns block objects. The second parameter is optional.
If specified, the notifications include whole transaction objects, Otherwise, the
...
chain with the `removed` property in the log object set to `true`. This means the subscription 
can publish notifications for multiple logs for the same transaction.
...
If a chain reorganization occurs, Woop resubmits transactions for inclusion in the new canonical
